...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    # Calculate hinge loss for each data point and sum them up
    total_loss = 0
...
def average_perceptron(feature_matrix, labels, T):
    ...
    # Calculate averages
    avg_theta = theta_sum / (T * n_samples)
    avg_theta_0 = theta_0_sum / (T * n_samples)
...
def classifier_accuracy(
    ...
    # Classify training data and calculate training accuracy
    train_preds = classify(train_feature_matrix, theta, theta_0)
    train_accuracy = accuracy(train_preds, train_labels)

    # Classify validation data and calculate validation accuracy
...
def bag_of_words(texts, remove_stopword=True):
    ...
    with open('stopwords.txt', 'r') as f:
        stopword = set(f.read().splitlines()) if remove_stopword else set()
